...
public sealed class KPIPagamentiScontoKPMGQueryPersistence(KPIPagamentiScontoKPMGQuery command) : DapperBase, IQuery<IEnumerable<KPIPagamentiScontoKPMGDto>>
{
    private readonly KPIPagamentiScontoKPMGQuery _command = command;


    private static readonly string _sql = KPIPagamentiSQLBuilder.SelectScontoKPMG();
    private static readonly string _orderBy = KPIPagamentiSQLBuilder.GrouByOrderScontoKPMG(); 
    public async Task<IEnumerable<KPIPagamentiScontoKPMGDto>> Execute(IDbConnection? connection, string schema, IDbTransaction? transaction, CancellationToken cancellationToken = default)
    {
        List<string> where = [];
 ...
 (c.provider_names LIKE '%'+ @abi +',%' 
  ...
  OR c.abi = 'ABI' + @abi) ");
        ...
        return await ((IDatabase)this).SelectAsync<KPIPagamentiScontoKPMGDto>(
            connection!,
            sql,
            parameters,
            transaction,
