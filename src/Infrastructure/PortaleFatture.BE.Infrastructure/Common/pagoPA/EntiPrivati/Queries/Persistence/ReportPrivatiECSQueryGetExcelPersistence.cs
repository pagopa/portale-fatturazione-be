...
using PortaleFatture.BE.Infrastructure.Common.pagoPA.EntiPrivati.Queries.Persistence.Builder;
using PortaleFatture.BE.Infrastructure.Common.Persistence;

namespace PortaleFatture.BE.Infrastructure.Common.pagoPA.EntiPrivati.Queries.Persistence;

public sealed class ReportPrivatiECSQueryGetExcelPersistence(ReportPrivatiECQueryGetExcel command) : DapperBase, IQuery<IEnumerable<ReportPrivatiECDto>>
{
    private readonly ReportPrivatiECQueryGetExcel _command = command; 

    private static readonly string _sql = ReportPrivatiECSQLBuilder.SelectEC();
    private static readonly string _groupBy = ReportPrivatiECSQLBuilder.GroupByEC();
    private static readonly string _orderBY = ReportPrivatiECSQLBuilder.OrderByEC();
    public async Task<IEnumerable<ReportPrivatiECDto>> Execute(IDbConnection? connection, string schema, IDbTransaction? transaction, CancellationToken cancellationToken = default)
    {
        List<string> where = [];
        ...
        where.AddInOrder(" p.year_quarter IN @Quarters");
 ...
 (c.provider_names LIKE '%'+ @abi +',%' 
  ...
  OR c.abi = 'ABI' + @abi) ");
        ...
        var sql = _sql + sWhere + _groupBy + _orderBY;

        return await ((IDatabase)this).SelectAsync<ReportPrivatiECDto>(
            connection!,
            sql,
            parameters,
            transaction,
