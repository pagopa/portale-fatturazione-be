...
public sealed class ReportPrivatiVBSQueryGetExcelPersistence(ReportPrivatiVBSQueryGetExcel command) : DapperBase, IQuery<IEnumerable<ReportPrivatiVBSDto>>
{
    private readonly ReportPrivatiVBSQueryGetExcel _command = command; 

    private static readonly string _sql = ReportPrivatiVBSSQLBuilder.SelectVBS();
    private static readonly string _groupBy = ReportPrivatiVBSSQLBuilder.GroupByVBS();
    public async Task<IEnumerable<ReportPrivatiVBSDto>> Execute(IDbConnection? connection, string schema, IDbTransaction? transaction, CancellationToken cancellationToken = default)
    {
        List<string> where = [];
 ...
 (c.provider_names LIKE '%'+ @abi +',%' 
  ...
  OR c.abi = 'ABI' + @abi) ");
        ...
        var sql = _sql + sWhere + _groupBy;

        return await ((IDatabase)this).SelectAsync<ReportPrivatiVBSDto>(
            connection!,
            sql,
            parameters,
            transaction,
