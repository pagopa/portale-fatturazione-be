...
public sealed class FinancialReportQueryGetKPMGReportExcelPersistence(FinancialReportQueryGetKPMGReportExcel command) : DapperBase, IQuery<IEnumerable<KPMGReportDto>>
{
    private readonly FinancialReportQueryGetKPMGReportExcel _command = command;


    private static readonly string _sql = FinancialReportSQLBuilder.SelectKPMGReport();
    private static readonly string _orderBy = FinancialReportSQLBuilder.OrderByKPMGReport();
    public async Task<IEnumerable<KPMGReportDto>> Execute(IDbConnection? connection, string schema, IDbTransaction? transaction, CancellationToken cancellationToken = default)
    {
        List<string> where = [];
 ...
 (c.provider_names LIKE '%'+ @abi +',%' 
  ...
  OR c.abi = 'ABI' + @abi) ");
        ...
        return await ((IDatabase)this).SelectAsync<KPMGReportDto>(
            connection!,
            sql,
            parameters,
            transaction,
