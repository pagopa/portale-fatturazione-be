...
public class ContestazioniRecapQueryPersistenceIntegration(ContestazioniRecapQueryIntegration command) : DapperBase, IQuery<IEnumerable<ContestazioneRecap>?>
{
    private readonly ContestazioniRecapQueryIntegration _command = command;
    private static readonly string _sql = ContestazioniMassiveSQLBuilder.SelectRecapIntegration();
    private static readonly string _gorderBy = ContestazioniMassiveSQLBuilder.GroupByOrderByRecapIntegration();
    public async Task<IEnumerable<ContestazioneRecap>?> Execute(IDbConnection? connection, string schema, IDbTransaction? transaction, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrEmpty(_command.ContractId))
        {
            ...
            parameters.ContractId = _command.ContractId;
        }
