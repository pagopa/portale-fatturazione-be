...
{
    public class FattureCreditoSospesoQueryPersistence(FattureCreditoSospesoQuery command) : DapperBase, IQuery<IEnumerable<FattureCreditoSospesoRawDto>>
    {
        ...
        public async Task<IEnumerable<FattureCreditoSospesoRawDto>> Execute(IDbConnection? connection, string schema, IDbTransaction? transaction, CancellationToken cancellationToken = default)
        {
            ...
            return await ((IDatabase)this).SelectAsync<FattureCreditoSospesoRawDto>(
                connection!, _sql,
                new
                {
                    _command.IdEnte,
