...
namespace PortaleFatture.BE.Infrastructure.Common.SEND.Fatture.QueryHandlers;
        ...
        ILogger<FattureDettaglioEmessoPdfAdminQueryHandler> logger) : IRequestHandler<FattureDettaglioEmessoPdfAdminQuery, IEnumerable<FatturaDocContabileEmessoDettaglioDto>>
    {
        ...
        private readonly ILogger<FattureDettaglioEmessoPdfAdminQueryHandler> _logger = logger;
        private readonly IStringLocalizer<Localization> _localizer = localizer;
        public async Task<IEnumerable<FatturaDocContabileEmessoDettaglioDto>> Handle(FattureDettaglioEmessoPdfAdminQuery request, CancellationToken ct)
        {
            using var rs = await _factory.Create(cancellationToken: ct);
            var rawResult = await rs.Query(new FattureDettaglioEmessoPdfAdminPersistence(request), ct);
