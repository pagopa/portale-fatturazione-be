...
{
    public class DatiModuloCommessaQueryDocumentoPagoPAHandler(
     IMediator handler,
     IStringLocalizer<Localization> localizer,
     ILogger<DatiModuloCommessaQueryDocumentoPagoPAHandler> logger) : IRequestHandler<DatiModuloCommessaDocumentoPagoPAQueryGet, ModuloCommessaDocumentoDto?>
    {
        private readonly IMediator _handler = handler;
        private readonly ILogger<DatiModuloCommessaQueryDocumentoPagoPAHandler> _logger = logger;
        private readonly IStringLocalizer<Localization> _localizer = localizer;

        public async Task<ModuloCommessaDocumentoDto?> Handle(DatiModuloCommessaDocumentoPagoPAQueryGet command, CancellationToken ct)
        {
            ...
            DatiFatturazione? datiFatturazione = new()
            {
                Contatti = []
            };
            try
            {
                datiFatturazione = await _handler.Send(new DatiFatturazioneQueryGetByIdEnte(authInfo), ct);
            }
            catch { } //sopprimo l'errore solo per azure/AD
