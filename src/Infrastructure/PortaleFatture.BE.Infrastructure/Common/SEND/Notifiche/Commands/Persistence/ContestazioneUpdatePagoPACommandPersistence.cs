...
public class ContestazioneUpdatePagoPACommandPersistence(ContestazioneUpdatePagoPACommand command, IStringLocalizer<Localization> localizer) : DapperBase, ICommand<int>
{
    public bool RequiresTransaction => false;
    private readonly ContestazioneUpdatePagoPACommand _command = command;
    IStringLocalizer<Localization> _localizer = localizer;

    private static readonly string _sqlUpdate = @"
UPDATE [schema][Contestazioni]
   SET [NoteSend] = @NoteSend  
      ,[FkIdFlagContestazione] = @StatoContestazione
      ,[Onere] = @Onere 
      ,[DataInserimentoSend] = @DataInserimentoSend 
      ,[DataModificaSend] = @DataModificaSend 
