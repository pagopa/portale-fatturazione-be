...
public class ContestazioneUpdateRecapitistaCommandHandler(
 IFattureDbContextFactory factory,
 IMediator handler,
 IStringLocalizer<Localization> localizer,
 ILogger<ContestazioneUpdateRecapitistaCommandHandler> logger) : IRequestHandler<ContestazioneUpdateRecapitistaCommand, Contestazione?>
{
    private readonly IFattureDbContextFactory _factory = factory;
    private readonly IMediator _handler = handler;
    private readonly ILogger<ContestazioneUpdateRecapitistaCommandHandler> _logger = logger;
    private readonly IStringLocalizer<Localization> _localizer = localizer;

    public async Task<Contestazione?> Handle(ContestazioneUpdateRecapitistaCommand command, CancellationToken ct)
    {
        ...
        if (notifica.Recapitista != command.AuthenticationInfo!.IdEnte)
        ...
        if (contestazione.DataInserimentoRecapitista == null)
            command.DataInserimentoRecapitista = adesso;
        else
        {
            command.DataInserimentoRecapitista = contestazione.DataInserimentoRecapitista;
            command.DataModificaRecapitista = adesso;
        }
        ...
        {
            if (command.StatoContestazione == (short)StatoContestazione.RispostaRecapitista && azione.RispostaPermessa)
            {
            ...
            {
                command.Onere = Profilo.Recapitista;
                command.NoteRecapitista = command.NoteRecapitista??contestazione.NoteRecapitista;
                command.ExpectedStatoContestazione = notifica.StatoContestazione;
        ...
        {
            var id = await uow.Execute(new ContestazioneUpdateRecapitistaCommandPersistence(command, _localizer), ct);
            if (id > 0)
            {
