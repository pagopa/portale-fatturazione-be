...
public class PagoPAHttpClient(
 IPortaleFattureOptions options,
 IHttpClientFactory httpClientFactory,
 ILogger<PagoPAHttpClient> logger) : IPagoPAHttpClient
{
    private readonly ILogger<PagoPAHttpClient> _logger = logger;
    ...
    public async Task<List<CertificateKey>?> GetCertificatesAsync(CancellationToken ct = default)
    {
        try
        {
            using var client = GetClient(_options.AzureAd!.Instance);
            var result = await client.GetAsync($"{_options.AzureAd!.TenantId}/discovery/keys?appid={_options.AzureAd!.ClientId}", ct);
