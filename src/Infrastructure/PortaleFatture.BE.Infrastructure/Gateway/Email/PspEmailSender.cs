...
public class PspEmailSender(
    string? accessToken,
    string? refreshToken,
    string? clientId,
    string? clientSecret,
    string? from,
    string? fromName) : IPspEmailSender
{
    ...
    public Dictionary<string, bool> SentFolder(
        ...
        string applicationName)
    {
        ...
        try
        {
            ...
            var credential = GoogleCredential.FromAccessToken(myaccessToken).CreateScoped(new[]
            {
            ...
            {
                var sentMessagesRequest = service.Users.Messages.List("me");
                sentMessagesRequest.LabelIds = "SENT";
                sentMessagesRequest.Q = $"rfc822msgid:{result.Value}";

                var sentMessagesResponse = sentMessagesRequest.Execute();
                //foreach (var message in sentMessagesResponse.Messages)
                //{
                //    results.Add(message.Id, false);
                //    //Console.WriteLine($"Message ID: {message.Id}");
                //}
                if (sentMessagesResponse.Messages != null && sentMessagesResponse.Messages.Any())
                    results.Add(result.Key, true);
                else
                    results.Add(result.Key, false);
            }
        }
        catch
        {
