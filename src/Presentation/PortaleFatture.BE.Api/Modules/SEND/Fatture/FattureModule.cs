...
public partial class FattureModule
{
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    private async Task<Results<Ok<FattureListaDto>, NotFound>> PostFattureSospeseByRicercaAsync(
    HttpContext context,
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    private async Task<IResult> PostFattureSospeseExcelByRicercaAsync(
    HttpContext context,
    [FromBody] FatturaSospeseRicercaRequest request,
    [FromServices] IStringLocalizer<Localization> localizer,
    [FromServices] IMediator handler)
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    private async Task<Results<Ok<IEnumerable<string>>, NotFound>> GetAnniRiepilogoFattureAsync(
    HttpContext context,
    [FromServices] IStringLocalizer<Localization> localizer,
    [FromServices] IMediator handler)
    {
        ...
        var anni = await handler.Send(new FattureRiepilogoAnniQuery(authInfo));
        if (anni.IsNullNotAny())
            return NotFound();
        return Ok(anni);
    }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    private async Task<Results<Ok<IEnumerable<FattureMeseResponse>>, NotFound>> PostMesiRiepilogoFattureAsync(
    HttpContext context,
    [FromBody] FattureMesiRequest request,
    [FromServices] IStringLocalizer<Localization> localizer,
    [FromServices] IMediator handler)
    {
        ...
        var mesi = await handler.Send(new FattureRiepilogoMesiQuery(authInfo)
        {
            Anno = request.Anno?.ToString()
        });
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    private async Task<Results<Ok<IEnumerable<int>>, BadRequest, NotFound>> GetPagoPAModificaGestioneFatturazioneAnniAsync(
    HttpContext context,
    GestioneFattureModificaRequest request,
    [FromServices] IMediator handler)
    {
        var authInfo = context.GetAuthInfo();
        var anni = await handler.Send(new GestioneFattureModificaAnniQuery(authInfo)
        {
            TipologiaFattura = request.TipologiaFattura,
            Azione = request.Azione
        });
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    private async Task<Results<Ok<IEnumerable<FattureMeseResponse>>, BadRequest, NotFound>> GetPagoPAModificaGestioneFatturazioneMesiAsync(
    HttpContext context,
    GestioneFattureModificaRequest request,
    [FromServices] IMediator handler)
    {
        var authInfo = context.GetAuthInfo();
        var mesi = await handler.Send(new GestioneFattureModificaMesiQuery(authInfo)
        {   
            Anno = request.Anno,
            TipologiaFattura = request.TipologiaFattura,
            Azione = request.Azione
        });
        if (mesi.IsNullNotAny()) return NotFound();
