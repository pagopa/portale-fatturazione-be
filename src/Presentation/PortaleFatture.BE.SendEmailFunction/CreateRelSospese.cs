...
public class CreateRelSospese(ILoggerFactory loggerFactory)
{
    private readonly ILogger _logger = loggerFactory.CreateLogger<CreateRelSospese>();
    ...
    string extension = ".csv")
    {
        ...
        try
        {
            ...
            var blobName = $"{idTestata.Anno}/{idTestata.Mese}/{idTestata.TipologiaFattura}/{idTestata.IdEnte}/{idTestata.IdContratto}/Rel_Report di dettaglio_Sospese_{nomeEnte}_{idTestata.Mese}_{idTestata.Anno}{extension}";
    ...
    [Function("CreateRelSospese")]
    public async Task<string> RunAsync([ActivityTrigger] CreateRelSospeseDataRequest req)
    {
        ...
        try
        {
            ...
            var query = new RelTestataSospesaQueryGetByListaEnti(authInfo)
            {
                Anno = anno,
                Mese = mese,
                TipologiaFattura = tipologiafattura,
            ...
            if (!(allRels == null || allRels.Count == 0))
            {
                ...
                foreach (var rel in allRels.RelTestate!)
                {
                    ...
                    var command = new RelRigheSospeseQueryGetById(authInfo) // ? TODO =>
                    {
                    ...
                    var stream = await rels!.ToStream<RigheRelDto, RigheRelDtoPagoPAMap>(); // ? TODO 
