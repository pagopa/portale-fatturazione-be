...
public class ContestazioniArchivioGetAnniMesi(ILoggerFactory loggerFactory)
{
    private readonly ILogger _logger = loggerFactory.CreateLogger<ContestazioniArchivioGetAnniMesi>();

    [Function("ContestazioniArchivioGetAnniMesi")]
        ...
        FunctionContext context)
    {
        ...
        var anni = await mediator.Send(new NotificheAnniQuery(authInfo));
        ...
        else
        {
            ...
            {
                var mesi = await mediator.Send(new NotificheMesiQuery(authInfo)
                {
                    Anno = anno
                });

                var response = mesi!.Select(x => new NotificheAnniMesiResponse()
                {
