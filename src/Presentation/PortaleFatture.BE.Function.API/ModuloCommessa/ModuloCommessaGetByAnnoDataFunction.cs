...
public class ModuloCommessaGetByAnnoDataFunction(ILoggerFactory loggerFactory)
{
    private readonly ILogger _logger = loggerFactory.CreateLogger<ModuloCommessaGetByAnnoDataFunction>();

    [Function("ModuloCommessaGetByAnnoData")]
    ...
    { 
        ...
        var modulo = await mediator.Send(new DatiModuloCommessaQueryGetByAnno(authInfo)
        {
        ...
        return modulo!.Select(x => x.Mapper())!; 
    }
}