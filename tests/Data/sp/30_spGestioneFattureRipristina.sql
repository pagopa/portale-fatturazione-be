/****** Oggetto: StoredProcedure [be].[spGestioneFattureRipristina]    Data dello script 23/07/2026 15:26:40 ******/
SET ANSI_NULLS ON
GO
...
/*
  ...
  Descrizione:           Ripristino fattura posticipata
  Target utilizzo:       Pulsante Ripristina della pagina PF "GestioneFatture"
  Versione:              1.0
*/
-- =============================================
...
BEGIN
    ...
    DECLARE @JobName VARCHAR(100) = 'RIPRISTINA FATTURA POSTICIPATA';
    ...
    BEGIN TRY
		...
		SELECT 
			...
			gf.Anno,
			gf.Mese,
			1 as Stato, -- RIPRISTINATA
			@IdUtente as IdUtenteRipristino,
			'RIPRISTINATA' as Azione,
			GETDATE() as DataRipristino,
			@Note as Note
		FROM cfg.GestioneFatture gf
		WHERE 
			gf.Stato = 0 -- Prendi solo le fatture posticipate
			AND
		...
		-- Controlla se esiste la fattura ed è in stato POSTICIPATA
		IF (
			@countFatture = 1
		)
		BEGIN            
			-- La fattura esiste ed è in stato POSTICIPATA
			...
			WHEN MATCHED THEN
				UPDATE 
					SET Stato = source.Stato
						,[DataRipristino] = source.[DataRipristino]
						,[IdUtenteRipristino] = source.[IdUtenteRipristino]
			...
			SET EndTime = @EndTime,
				...
				Description = CONCAT('Ripristinata Fattura: ',@stringFattura),
				Duration = DATEDIFF(SECOND,  @StartTime, @EndTime)
			WHERE JobId = @JobId
		...
		BEGIN
			...
			declare @err_description nvarchar(100) = CONCAT('La Fattura: ',@stringFattura,' non esiste in gestione fatture oppure è già stata ripristinata.')
