...
FROM cfg.GestioneFatture gf
    ...
    CROSS APPLY OPENJSON(gf.Note) WITH (Data datetime2 '$.Data', Testo nvarchar(max) '$.Testo') j
WHERE gf.Stato <> 2
GROUP BY gf.FkIdEnte, e.description, gf.FkTipologiaFattura, gf.Anno, gf.Mese, gf.Azione,
         gf.DataInserimento, gf.DataRipristino, tc.Descrizione, tc.IdTipoContratto;
